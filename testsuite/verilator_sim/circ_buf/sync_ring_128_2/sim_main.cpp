...
#include "Vsonic_sync_ring_128_2.h"		// From Verilating "sonic_blocksync_xg.sv"
...
Vsonic_sync_ring_128_2 *top;	// Instantiation of module
...
int main(int argc, char **argv, char **env) {
    ...
    top = new Vsonic_sync_ring_128_2;		// Create instance of module
    ...
    tfp->open ("Vsonic_sync_ring_128_2.vcd");	// Open the dump file
#endif
    ...
    long unsigned int li1, li2, li3, li4;
    char *pEnd;
    const char *str_c;
    ...
    while (main_time < SIM_TIME && !Verilated::gotFinish()) {
        ...
        if (!indata.eof() && top->wr_clock == 0 ){
            ...
            li3 = strtoul(pEnd, &pEnd, 16); //wr_address
            li4 = strtoul(pEnd, &pEnd, 16); //rd_address
        ...
        // Read outputs
#if INPUT_VEC
        VL_PRINTF ("[%" VL_PRI64 "d] %x %08X %08X %08X %08X %x \n",
               main_time, top->wr_clock, top->data_in[3], top->data_in[2], top->data_in[1], top->data_in[0], top->data_out);
#endif
