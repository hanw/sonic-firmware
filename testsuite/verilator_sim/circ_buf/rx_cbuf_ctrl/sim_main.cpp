...
#include "Vsonic_rx_ctl_66.h"		// From Verilating "sonic_rx_ctl_66.sv"
...
Vsonic_rx_ctl_66 *top;	// Instantiation of module
...
int main(int argc, char **argv, char **env) {
    ...
    top = new Vsonic_rx_ctl_66;		// Create instance of module
    ...
    tfp->open ("Vsonic_rx_ctl_66.vcd");	// Open the dump file
#endif
    top->data_in[1] = 0;
    top->data_in[0] = 0;
    top->wrena = 1;
    ...
    while (main_time < SIM_TIME && !Verilated::gotFinish()) {
        ...
        //cout << top->data_in[2] << top->data_in[1] << top->data_in[0] << endl;
        if (main_time > 4000 && main_time % 4 == 0 && top->rdena == 1 && top->rdreq == 1) {
        ...
        if (main_time > 8004 && main_time % 4 == 0) {
        ...
        // Read outputs
#if INPUT_VEC
        VL_PRINTF ("[%" VL_PRI64 "d] %x %x %08x %08x, %08x %08x \n",
               main_time, top->wr_clock, top->data_in[2], top->data_in[1], top->data_in[0], top->data_out[1], top->data_out[0]);
#endif
