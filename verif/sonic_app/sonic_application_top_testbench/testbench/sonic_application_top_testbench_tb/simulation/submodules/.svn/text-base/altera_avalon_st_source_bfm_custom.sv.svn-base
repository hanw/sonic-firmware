...
module altera_avalon_st_source_bfm_custom (
                                         clk,   
                                         reset,
...
// synthesis translate_off
   ...
   typedef logic [15:0]			    STBe0_t;
   typedef logic [7:0]                      STBardec0_t;
   ...
   STBe0_t			src_be0_temp, src_be0_slices;
   STBardec0_t			src_bardec0_temp, src_bardec0_slices;
   ...
   function automatic void push_transaction(); // public
      ...
      if (USE_VALID == 1) begin
          for (int i=0; i<new_transaction.idles; i++) begin
             ...
             idle_transaction.be0           = 'x;
             idle_transaction.bardec0       = 'x;
   ...
   function automatic void set_transaction_be0( // public
      STBe0_t be0
   );
      // Set the transaction data value
      $sformat(message, "%m: called set_transaction_be0 - %h", be0);
      print(VERBOSITY_DEBUG, message);
      new_transaction.be0 = be0;
   endfunction

   function automatic void set_transaction_bardec0( // public
      STBardec0_t bardec0
   );
      // Set the transaction data value
      $sformat(message, "%m: called set_transaction_bardec0 - %h", bardec0);
      print(VERBOSITY_DEBUG, message);
      new_transaction.bardec0 = bardec0;
   endfunction
   ...
   always @(*) begin
      ...
      if (USE_VALID > 0) begin
        if (USE_READY == 0 || ST_READY_LATENCY == 0) begin
            ...
            src_be0           = src_be0_temp; 
            src_bardec0       = src_bardec0_temp;
        end else begin
            ...
            if (transaction_pending) begin
               if (idle_ctr == 0) begin
                  ...
                  for (int i=0; i<ST_BEATSPERCYCLE; i++) begin
                     ...
                     src_be0_slices = src_be0_slices |
                         current_transaction[i].be0   << (i*16);  
                     src_bardec0_slices = src_bardec0_slices |
                         current_transaction[i].bardec0   << (i*8);  
                  end
                  ...
                  src_be0_temp   <= src_be0_slices;  
                  src_bardec0_temp   <= src_bardec0_slices;   
